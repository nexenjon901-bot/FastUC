...
const AdminLoginPage: React.FC<AdminLoginPageProps> = ({ onLogin }) => {
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, fontFamily: 'Outfit, sans-serif' }}>
      <div style={{ width: '100%', maxWidth: 420, background: C.card, border: 1px solid , borderRadius: 28, padding: '40px 32px' }}>
        ...
        <form onSubmit={handleLogin}>
          <div style={{ marginBottom: 16 }}>
            ...
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="admin@fastuc.uz" style={{ width: '100%', background: '#0d0f1e', border: 1.5px solid , borderRadius: 14, padding: '14px 16px', color: C.text, fontSize: '0.95rem', outline: 'none', boxSizing: 'border-box', fontFamily: 'Outfit, sans-serif' }} onFocus={e => (e.target.style.borderColor = C.accent)} onBlur={e => (e.target.style.borderColor = C.border)} />
          </div>
          <div style={{ marginBottom: 24 }}>
            <label style={{ color: C.muted, fontSize: '0.78rem', fontWeight: 600, display: 'block', marginBottom: 6 }}>Parol</label>
            <div style={{ position: 'relative' }}>
              <input type={showPass ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" style={{ width: '100%', background: '#0d0f1e', border: 1.5px solid , borderRadius: 14, padding: '14px 48px 14px 16px', color: C.text, fontSize: '0.95rem', outline: 'none', boxSizing: 'border-box', fontFamily: 'Outfit, sans-serif' }} onFocus={e => (e.target.style.borderColor = C.accent)} onBlur={e => (e.target.style.borderColor = C.border)} />
